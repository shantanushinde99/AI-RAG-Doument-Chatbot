...
api_key = st.sidebar.text_input(
    "Groq API Key",
    type="password",
    help="Required for Groq LLaMA services.",
)
...
def get_conversational_chain():
    """
    Creates QA chain with custom prompt using Groq LLaMA model.
    """
    prompt_template = """
    ...
    **Answer**:
    ...
    Answer:
    """
    model = ChatGroq(model_name="llama-3.3-70b-versatile", temperature=0.3, api_key=os.environ.get("GROQ_API_KEY"))
    prompt = PromptTemplate(template=prompt_template, input_variables=["context", "question"])
    return load_qa_chain(model, chain_type="stuff", prompt=prompt)

def get_theme_summary(docs, question):
    """
    Generates a summary of recurring themes using Groq LLaMA model.
    """
    prompt_template = """
    ...
    **Answer**:
    ...
    Summary:
    """
    model = ChatGroq(model_name="llama-3.3-70b-versatile", temperature=0.5, api_key=os.environ.get("GROQ_API_KEY"))
    prompt = PromptTemplate(template=prompt_template, input_variables=["context", "question"])
...
st.markdown(
    ...
    <div class="text-center text-gray-500 mt-8">
        ...
        <p>Powered by LangChain, Groq LLaMA, and FAISS</p>
    </div>
    """,
    unsafe_allow_html=True,
)